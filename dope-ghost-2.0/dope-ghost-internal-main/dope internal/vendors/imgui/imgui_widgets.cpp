...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	// Draw frame
	RenderNavHighlight(frame_bb, id);
	RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2{ 2.f, 0.f }, GetColorU32(ImGuiCol_ChildBg), true, g.Style.FrameRounding);
    ...
    // hover anim
	//{
	//	static std::map<ImGuiID, float> times{};
	//	if (times.find(id) == times.end())
	//		times.emplace(id, 0.f);

	//	auto& actualTime = times.at(id);
	...
	//	ImVec4 hovercol = ImColor(181, 198, 240, 10);

	//	if (actualTime > 0.f)
	//		RenderFrame(ImVec2{ frame_bb.Min.x, frame_bb.Max.y - (label_size.y + style.FramePadding.y * 2.0f) * easeInOutSine(actualTime) }, frame_bb.Max - ImVec2{ 2.f, 0.f }, GetColorU32(hovercol), true, style.FrameRounding);
	//}
	...
	{
		static std::map<ImGuiID, float> times{};
		static std::map<ImGuiID, float> poses{};
		static std::map<ImGuiID, float> newPoses{};
		static std::map<ImGuiID, bool> changings{};

		if (times.find(id) == times.end())
			times.emplace(id, 0.f);
		if (poses.find(id) == poses.end())
			poses.emplace(id, 0.0f);
		if (newPoses.find(id) == newPoses.end())
			newPoses.emplace(id, 0.0f);

		auto& actualTime = times.at(id);
		auto& actualPos = poses.at(id);
		auto& actualNewPos = newPoses.at(id);

		if (changings.find(id) == changings.end())
		{
			actualTime = 0.f;
			actualNewPos = grab_bb.Max.x;
			actualPos = (actualNewPos - actualPos);
			changings.emplace(id, true);
		}

		auto& actualChanging = changings.at(id);
		...
		{
			actualTime += g.IO.DeltaTime / 2;
			actualPos += (float)((actualNewPos - actualPos) * easeOutExpo(actualTime));
		}
		...
		RenderFrame(frame_bb.Min + ImVec2{ 2.f, 2.f }, ImVec2{ actualChanging ? actualPos : grab_bb.Max.x, frame_bb.Max.y - 2.f }, GetColorU32(ImGuiCol_SliderGrab), false, style.FrameRounding);
	}
