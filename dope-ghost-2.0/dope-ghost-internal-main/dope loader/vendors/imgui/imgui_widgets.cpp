...
static std::map<ImGuiID, float> checkbox_clicked{};
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    {
        if (checkbox_clicked.find(id) == checkbox_clicked.end())
            checkbox_clicked.emplace(id, 1.f);

        auto& curTime = checkbox_clicked.at(id);

        if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
            curTime = ImSaturate(g.LastActiveIdTimer / 0.1f);
        }

        float t = *v ? (curTime) : (1.0f - curTime);
        ...
        {
            auto check = GetColorU32(ImGuiCol_CheckMark);
            draw_list->AddRectFilled(ImVec2(p.x, p.y + 6), ImVec2((p.x - 6) + width, p.y + height - 6), *v ? GetColorU32(ImVec4(ImColor(54, 83, 204))) : GetColorU32(ImVec4(ImColor(51, 56, 83))), height * 0.5f);
            draw_list->AddCircleFilled(ImVec2((p.x - 3) + radius + t * (width - radius * 2.0f), p.y + radius), radius - 2.5f, *v ? GetColorU32(ImVec4(155.f / 255.f, 169.f / 255.f, 230.f / 255.f, 1.f)) : GetColorU32(check));
        }
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2{ 2.f, 0.f }, ImU32(ImColor(ImVec4(30.f / 255.f, 27.f / 255.f, 30.f / 255.f, 1.f))), false, 5.f);
